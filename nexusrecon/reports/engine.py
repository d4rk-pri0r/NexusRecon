...
# ── Honest finding provenance (ROADMAP item 6) ────────────────────────────────

_MOCK_FINDING_MARKER = "[MOCK: templated output, not analysis]"


def finding_is_mock(f: dict[str, Any]) -> bool:
    """True when a finding came from the keyless MockLLM fallback.

    Driven by the robust ``analysis_engine`` marker the agent executor stamps on
    every finding; falls back to the legacy ``source == 'mock_llm'`` for findings
    produced before that marker existed."""
    return f.get("analysis_engine") == "mock" or f.get("source") == "mock_llm"


def finding_display_title(f: dict[str, Any], default: str = "Untitled") -> str:
    """Finding title for human-facing reports, prefixed with an unmistakable
    marker when the finding is templated MockLLM output rather than reasoned
    analysis. Never mutates the underlying finding data."""
    title = f.get("title") or default
    if finding_is_mock(f) and not str(title).startswith("[MOCK"):
        return f"{_MOCK_FINDING_MARKER} {title}"
    return str(title)


def finding_evidence_hash_label(f: dict[str, Any]) -> str:
    """Render the evidence-hash value with an honest qualifier when the hash is
    self-reported (a content hash of the finding's own prose) rather than a hash
    of independently collected raw evidence (ROADMAP item 6)."""
    h = f.get("raw_evidence_hash", "N/A")
    if f.get("evidence_provenance") == "self_reported":
        return f"{h} (content hash of finding text; self-reported, not independent evidence)"
    return str(h)
...
class ReportEngine:
    ...
    def _executive_summary(self, state: dict[str, Any]) -> str:
        ...
        for i, f in enumerate(findings[:10], 1):
            lines.append(f"{i}. **[{f.get('severity', 'info').upper()}]** {finding_display_title(f)}")
    ...
    def _full_report(self, state: dict[str, Any]) -> str:
        ...
        for i, f in enumerate(findings, 1):
            lines.extend([
                ...
                f"- **Evidence Hash:** {finding_evidence_hash_label(f)}",
                "",
                f"**Description:** {f.get('description', '')}",
                "",
            ])
            if finding_is_mock(f):
                lines.extend([
                    "> Note: generated by the keyless MockLLM fallback (templated "
                    "output, not reasoned analysis). Configure an LLM API key for "
                    "real analysis.",
                    "",
                ])
    ...
    def _attack_surface(self, state: dict[str, Any]) -> str:
        ...
        for i, f in enumerate(findings, 1):
            ...
            lines.append(
                ...
                f"{f.get('confidence', 0):.0%} | {finding_display_title(f, '')} | "
    ...
    def _vuln_correlation(self, state: dict[str, Any]) -> str:
        ...
        if findings:
            ...
            lines.append("")
            for i, f in enumerate(findings, 1):
                lines.append(f"### {i}. {finding_display_title(f)}")
    ...
    def _jira_tracker(self, state: dict[str, Any]) -> str:
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            ...
            for finding in findings:
                writer.writerow([
                    finding_display_title(finding, "Untitled Finding"),
    ...
    def _pdf_report(self, state: dict[str, Any]) -> str:
        ...
        for i, f in enumerate(findings[:15], 1):
            ...
            title = finding_display_title(f, "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    ...
    def _pdf_report_fallback(self, state: dict[str, Any]) -> str:
        ...
        for f in findings[:30]:
            _t = finding_display_title(f, "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            lines.append(f'<li><b>[{f.get("severity", "info").upper()}]</b> {_t}</li>')
    ...
    def _pptx_report(self, state: dict[str, Any]) -> str:
        ...
        for i, f in enumerate(findings[:8], 1):
            top_findings_bullets.append(
                f"{i}. [{f.get('severity', 'info').upper()}] {finding_display_title(f, '')}"
            )
