...
Tools that need API keys are automatically skipped when their key is
missing, so the OSINT phase layer works with zero configuration. The
LLM synthesis layer requires one credential source: `NEXUS_LLM_AUTH_MODE`
defaults to `auto`, which prefers the selected cloud provider's
official-CLI subscription login and falls back to that provider's API
key when one is set. A cloud provider with neither a login nor a key
raises an actionable error naming the exact login command and the key
variable — the deterministic `MockLLM` runs only when you explicitly
set `NEXUS_LLM_PROVIDER=mock`, never as a silent fallback.
...
```env
NEXUS_LLM_PROVIDER=anthropic   # anthropic | openai | openai-codex | xai | ollama | mock
NEXUS_LLM_AUTH_MODE=auto       # auto | oauth | api_key (default: auto)
NEXUS_LLM_CLI_TIMEOUT=600      # seconds per official-CLI call (1-3600)
NEXUS_LLM_MODEL=claude-opus-4-5
NEXUS_LLM_TEMPERATURE=0.1
ANTHROPIC_API_KEY=             # optional; metered billing / api_key mode
OPENAI_API_KEY=                # optional
XAI_API_KEY=                   # optional; direct endpoint https://api.x.ai/v1
OLLAMA_BASE_URL=http://localhost:11434  # Required for local Ollama
OLLAMA_MODEL=llama3.1:8b
```

With `NEXUS_LLM_AUTH_MODE=auto` (the default), a cloud provider resolves
in this order: official-CLI subscription login → that provider's API key
(if set) → actionable error naming the login command and key variable.
`api_key` forces the direct SDK client. Subscription inference records
zero metered USD in the campaign budget (token telemetry is preserved);
API-key calls retain normal pricing.

### Provider OAuth: official-CLI subscription login

First-class cloud providers delegate login, token refresh, storage, and
inference to their maintained official CLI. Install the CLI you need
with a safe package-manager command (never pipe-to-shell):

```bash
npm install -g @anthropic-ai/claude-code   # anthropic (Claude Code)
npm install -g @openai/codex               # openai / openai-codex (Codex CLI)
npm install -g @xai-official/grok          # xai (Grok Build)
```

Log in once per provider — the official CLI's browser or auth-code flow
streams directly to your terminal:

```bash
nexusrecon auth login anthropic                # Claude.ai subscription (Pro/Max); no --device variant
nexusrecon auth login openai-codex             # Codex / ChatGPT browser login
nexusrecon auth login openai-codex --device    # device-code login
nexusrecon auth login xai                      # browser login
nexusrecon auth login xai --device             # device-code login
```

Check readiness (`--provider` accepts `anthropic`, `openai`,
`openai-codex`, `xai`, or `all`; it never prints tokens, keys, or
credential values):

```bash
nexusrecon auth status --provider anthropic
```

Each CLI stores credentials in its own native store — NexusRecon never
copies or stores them:

| Provider | Binary | Native credential store | Override env var |
|---|---|---|---|
| `anthropic` | `claude` | Claude Code system credential store, or `~/.claude/.credentials.json` | `CLAUDE_CONFIG_DIR` |
| `openai` | `codex` | OS keyring, or `~/.codex/auth.json` | `CODEX_HOME` |
| `xai` | `grok` | `~/.grok/auth.json` | `GROK_HOME` |

Operational notes:

- **First run:** the native OAuth credentials must exist before an
  actual live campaign — run the login once, then confirm with
  `nexusrecon auth status` before launching.
- **xai readiness is store-presence only** (no documented status probe):
  a present `~/.grok/auth.json` is reported as "credential present
  (validated on use)"; Grok validates/refreshes it at first inference.
- **OAuth inference is text-only** in this release. Image content fails
  closed with an instruction to switch to `NEXUS_LLM_AUTH_MODE=api_key`
  with the provider's key (or Ollama) for vision — it is never silently
  dropped.
- Inference subprocesses run in a private temporary directory with
  tools disabled/read-only, provider API-key env vars stripped, and no
  prompt in argv (xai receives a mode-`0600` prompt file, deleted after
  the run).
...
**`agent_executor.py`**, `AgentExecutor` bridges the LLM and the phase nodes. `run_agent(agent_name, task_data, task_prompt)` instantiates the named agent class, builds a prompt incorporating the agent's `role`, `goal`, and `backstory`, calls the LLM, and returns the analysis string. The client is built by `nexusrecon.llm.factory` per the auth-mode rules in §3 — a cloud provider with neither an official-CLI login nor an API key raises an actionable `LLMAuthenticationError` rather than silently degrading to `MockLLM` (the mock runs only when `NEXUS_LLM_PROVIDER=mock` is set explicitly).
...
### `No usable <provider> authentication is available`

The selected cloud provider has neither an official-CLI subscription
login nor an API key. Install the provider's official CLI
(`npm install -g @anthropic-ai/claude-code`, `npm install -g
@openai/codex`, or `npm install -g @xai-official/grok`), log in with
`nexusrecon auth login <provider>`, and check readiness with
`nexusrecon auth status --provider <provider>`. Alternatively set the
provider's API key (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or
`XAI_API_KEY`). To run deliberately without an LLM, set
`NEXUS_LLM_PROVIDER=mock` explicitly.
